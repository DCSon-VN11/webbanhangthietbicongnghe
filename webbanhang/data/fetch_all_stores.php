...
$productId = $_GET['product_id']; // Lấy ID sản phẩm từ yêu cầu

// Câu truy vấn lấy tất cả cửa hàng có sản phẩm
$sql = "SELECT dc.*, kh.Hangton FROM diachicuahang dc 
        JOIN khohang kh ON dc.diachicuahangid = kh.DiaChiCuaHangID 
        WHERE kh.Hangton > 0 AND kh.Hangton > 0 AND kh.SanPhamID = '$productId'";
